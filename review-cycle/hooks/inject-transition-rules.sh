...
# review-record.md status transitions, also read by state-gate.sh) and the
# current state out of review-record.md, and emits a compact block naming
...
# the defect docs/reports/2026-07-26-hunt-conversational-state-machine.md
# reproduces and this hook exists to prevent.
#
...
fail_block() {
  # $1: reason line(s)
  cat <<EOF
<review-transition-rules status="error">
...
rules_file = os.environ["REVIEW_RULES_FILE"]
state_path = os.environ["REVIEW_STATE_PATH"]
...
cat <<EOF
<review-transition-rules status="ok">
$body
