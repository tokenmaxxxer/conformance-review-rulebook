...
deny() { echo "review: refused — $1" >&2; exit 2; }
...
REVIEW_CPD="${CLAUDE_PROJECT_DIR:-}" \
REVIEW_CWD="$(pwd -P 2>/dev/null || echo)" \
python3 <<'PY'
import json, os, posixpath, re, shlex, subprocess, sys

def deny(msg):
    sys.stderr.write("review: refused — %s\n" % msg)
    sys.exit(2)
...
def _review_fail_closed(_t, _v, _tb):
    try:
        sys.stderr.write("review: refused — fail-closed: internal error (%s: %s)\n" % (_t.__name__, _v))
    except Exception:
        pass
    os._exit(2)
sys.excepthook = _review_fail_closed

raw = os.environ.get("REVIEW_PAYLOAD", "")
try:
...
cpd = os.environ.get("REVIEW_CPD", "")
cwd = os.environ.get("REVIEW_CWD", "") or "."
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "review: refused — fail-closed: internal error (gate judge exited $rc)" >&2
  exit 2
fi
exit "$rc"
