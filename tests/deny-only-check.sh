...
# --- substance probe: an empty review record must be refused --------------
probe_dir="${1:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../review/hooks" && pwd -P)}"
rec_rel="docs/issue-999/reports/review.md"
...
substance_probe() {
  ...
  rm -rf "$td"
  if [ "$refused" = 0 ]; then
    echo "deny-only-check: FAIL — no gate refuses an empty review record at $rec_rel (contract s20)" >&2
    return 1
  fi
  return 0
}
