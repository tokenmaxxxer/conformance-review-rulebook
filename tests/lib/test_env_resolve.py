...
Vendored verbatim from docs/specs/test-env-resolution.md (on-the-record,
issue #551). The on-the-record sibling checkout is read-only on this
machine (git push/fetch blocked), so this repo cannot cross-repo `import`
gates.test_env_resolve directly — it vendors the reference module instead,
invoked as the spec's documented Bash-test-runner CLI shape
(docs/issue-51/proposals/2026-08-09-adopt-test-env-resolution.md).
...
"the gate under test actually regressed".
  ...
  python3 test_env_resolve.py <candidate1> <candidate2> ...
"""
